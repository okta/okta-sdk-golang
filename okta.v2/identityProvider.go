...
func (m *IdentityProviderResource) GetCsrForIdentityProvider(idpId string, csrId string) (*CSRMetadata, *Response, error) {
	url := fmt.Sprintf("/api/v1/idps/%v/credentials/csrs/%v", idpId, csrId)
	...
	var csrMetadata *CSRMetadata

	resp, err := m.client.requestExecutor.Do(req, &csrMetadata)
	if err != nil {
		return nil, resp, err
	}

	return csrMetadata, resp, nil
}
