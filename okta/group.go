...
func (m *GroupResource) UpdateGroup(groupId string, body Group)  *Group {
...
func (m *GroupResource) DeleteGroup(groupId string)  {
...
func (m *GroupResource) RemoveGroupUser(groupId string, userId string)  {
	_, err := m.client.requestExecutor.Delete("/api/v1/groups/"+groupId+"/users/"+userId+"")
	if err != nil  {
		fmt.Printf("error: %v", err)
	}
}
func (m *GroupResource) ListGroupUsers(groupId string)  *User {
