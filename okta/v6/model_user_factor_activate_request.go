...
// PushAsUserFactorActivateRequest is a convenience function that returns Push wrapped in UserFactorActivateRequest
func PushAsUserFactorActivateRequest(v *Push) UserFactorActivateRequest {
	return UserFactorActivateRequest{
		Push: v,
	}
}
...
func (dst *UserFactorActivateRequest) UnmarshalJSON(data []byte) error {
	...
	// try to unmarshal data into Push
	err = json.Unmarshal(data, &dst.Push)
	if err == nil {
		jsonPush, _ := json.Marshal(dst.Push)
		if string(jsonPush) == "{}" { // empty struct
			dst.Push = nil
		} else {
			match++
		}
...
func (obj UserFactorActivateRequest) GetActualInstanceValue() interface{} {
	if obj.Call != nil {
		return *obj.Call
	}
