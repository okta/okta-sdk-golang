...
func TestGetOrgContactTypes(t *testing.T) {
	...
	contactTypes, _, err := client.OrgContactTypeObj.GetOrgContactTypes(ctx)
	require.NoError(t, err)
...
func TestGetAndUpdateOrgContactUser(t *testing.T) {
	...
	technicalUser, _, err := client.OrgContactUser.GetOrgContactUser(ctx, "TECHNICAL")
	require.NoError(t, err)

	billingUser, _, err := client.OrgContactUser.GetOrgContactUser(ctx, "BILLING")
	require.NoError(t, err)

	_, _, err = client.OrgContactUser.UpdateOrgContactUser(ctx, "TECHNICAL", okta.UserIdString{UserId: billingUser.UserId})
	require.NoError(t, err)

	_, _, err = client.OrgContactUser.UpdateOrgContactUser(ctx, "BILLING", okta.UserIdString{UserId: technicalUser.UserId})
	require.NoError(t, err)
}
...
func TestOptInOptOutOktaCommunicationSettings(t *testing.T) {
	...
	settings, _, err := client.OrgOktaCommunicationSetting.GetOktaCommunicationSettings(ctx)
	require.NoError(t, err)
	...
	if foundOptIn {
		_, _, err := client.OrgOktaCommunicationSetting.OptInUsersToOktaCommunicationEmails(ctx)
		require.NoError(t, err)
		_, _, err = client.OrgOktaCommunicationSetting.OptOutUsersFromOktaCommunicationEmails(ctx)
		require.NoError(t, err)
	}
...
// okta/orgOktaSupportSettingsObj.go
func TestGetGrantExtentRevokeOktaCommunicationSettings(t *testing.T) {
	...
	settings, _, err := client.OrgOktaSupportSettingsObj.GetOrgOktaSupportSettings(ctx)
	require.NoError(t, err)
	...
	for countOps < len(ops) {
		switch ops[opIndex] {
		case "revoke":
			_, _, err := client.OrgOktaSupportSettingsObj.RevokeOktaSupport(ctx)
			require.NoError(t, err)
		case "grant":
			_, _, err := client.OrgOktaSupportSettingsObj.GrantOktaSupport(ctx)
			require.NoError(t, err)
		case "extend":
			_, _, err := client.OrgOktaSupportSettingsObj.ExtendOktaSupport(ctx)
			require.NoError(t, err)
		}
...
func TestGetPerfsShowHideFooter(t *testing.T) {
	...
	preferences, _, err := client.OrgPreferences.GetOrgPreferences(ctx)
	require.NoError(t, err)
	...
	if showEndUserFooter {
		_, _, err = client.OrgPreferences.HideOktaUIFooter(ctx)
		require.NoError(t, err)
		_, _, err := client.OrgPreferences.ShowOktaUIFooter(ctx)
		require.NoError(t, err)
	}
